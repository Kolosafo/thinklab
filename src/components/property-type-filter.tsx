...
import { Building, Home, Hotel, Castle, Landmark, Store, Building2 } from "lucide-react"
...
const propertyTypes: PropertyType[] = [
  { id: "Apartment", name: "Apartment", icon: <Building className="h-5 w-5" /> },
  { id: "Duplex", name: "Duplex", icon: <Home className="h-5 w-5" /> },
  { id: "Semi-Detached", name: "Semi-Detached", icon: <Building2 className="h-5 w-5" /> },
...
export function PropertyTypeFilters({ activePropertyType }: PropertyTypeFiltersProps) {
    ...
    <ScrollArea className="w-full whitespace-nowrap">
      <div className="flex space-x-4 py-2">
        {propertyTypes.map((propertyType) => (
          <Button
            key={propertyType.id}
            variant={activePropertyType === propertyType.id ? "default" : "outline"}
